...
로컬 개발 서버를 `myapp.test` 같은 도메인 위에 올려주는 맥용 미니 앱.
벨보이가 손님을 방까지 안내하듯, 들어온 요청을 알맞은 로컬 서버로 데려다줍니다.
Caddy를 엔진으로 쓰고, `/etc/hosts` 편집·Caddy 프로세스 관리·경로별 라우팅 규칙을
GUI에서 한 번에 다룹니다. Ophiuchi(nginx 기반)의 Caddy 버전이라고 보면 됩니다.

## 다운로드

[**⬇️ 최신 버전 받기 (macOS Apple Silicon)**](https://github.com/soohyun-kwon/bellboy/releases/latest/download/Bellboy_aarch64.zip)

압축을 풀고 `Bellboy.app` 을 `Applications` 로 옮기면 됩니다. 전체 버전은 [releases](https://github.com/soohyun-kwon/bellboy/releases) 에서 볼 수 있습니다.

> **처음 실행 시** — 서명/공증되지 않은 앱이라 macOS가 한 번 막습니다.
> `Bellboy.app` 을 우클릭 → **열기** 를 누르거나, 터미널에서 아래를 실행하세요:
> ```bash
> xattr -dr com.apple.quarantine /Applications/Bellboy.app
> ```
> 엔진으로 [Caddy](https://caddyserver.com/)가 필요합니다 — 없으면 앱이 원클릭 설치를 안내합니다.
> Intel Mac은 사전 빌드가 없으니 아래 **개발 실행 / 빌드** 섹션대로 소스에서 빌드하세요.
...
- **Node.js TLS 자동 설정** — `~/.zshenv` 에 `NODE_EXTRA_CA_CERTS` + `NODE_OPTIONS=--use-system-ca` 를 넣어 Next.js SSR/렌더 워커의 로컬 HTTPS 인증서 오류 방지
- 외부 / 잔여 Caddy 프로세스 **감지·정리** (포트 80·443·2019 충돌 방지)
- 첫 권한 사용 시 sudoers + Touch ID 자동 설치 (이후 무프롬프트)
- 설정은 `~/Library/Application Support/bellboy/config.json`
- 생성되는 Caddyfile: `~/Library/Application Support/bellboy/Caddyfile`
...
| `hosts.rs` | `/etc/hosts` Bellboy 블록 동기화 |
